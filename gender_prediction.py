...
dataset = EEGGenderDataset(data_folder)
...
model = EEGConformer(
    n_outputs=2,           # Two classes: 0 (Female) and 1 (Male)
    n_chans=n_channels,
    n_times=n_times,
...
print(classification_report(all_test_labels, all_test_preds, target_names=['Female', 'Male']))
...
with torch.no_grad():
    for inputs, labels, filenames in report_loader_tqdm:
        ...
        outputs = model(inputs)
        _, preds = torch.max(outputs, 1)
        pred_gender = gender_mapping[preds.item()]
        true_gender = gender_mapping[labels.item()]
